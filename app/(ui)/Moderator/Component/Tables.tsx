...
import { fetchModeratorProjects } from '@/app/lib/actions';
...
export default async function Tables() {
  ...
  const datas = await fetchModeratorProjects();
        ...
        <tbody className='  flex-col  gap-3 flex'>
          {datas?.map((data)=>(
            ...
            w-full pr-3 items-center'>
                  <td className='max-[425px]:w-3/5 max-[375px]:w-4/6 max-[320px]:w-3/5 w-1/3 truncate '>{data.project.title}  </td>
