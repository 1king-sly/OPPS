...
export default   function Header() {
  ...
  return (
    <div className='w-full h-full flex items-center justify-around px-1 sm:px-4 lg:px-8 '>
      ...
      <div className='w-1/4 flex justify-end max-[420px]:hidden md:text-lg lg:text-xl text-xs'><span className='text-sky-400'> {session.data?.firstName || session.data?.email  } </span> </div>
    </div>
  )
}
